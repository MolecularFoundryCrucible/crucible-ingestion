...
def is_file_lost(message, dataset_to_process, ch, update_status=True):
    ...
    if not file_exists:
        if update_status:
            client.files.update_ingestion_status(dsid, reqid, status = "file not found")
        file_lost = True
...
def callback(ch, method, props, body):
    ...
    client.files.update_ingestion_status(dsid, reqid, status = "started", ingestion_githash = ingestion_githash)
    ...
    try:
        ...
        logger.info(f"{ds=}")
        if ds is None:
            client.files.update_ingestion_status(dsid, reqid, status = "not supported", ingestion_githash = ingestion_githash)    
            ...
            client.files.update_ingestion_status(dsid,
                                                 reqid, 
                                                 status = "complete",
